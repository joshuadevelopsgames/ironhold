...
/**
 * Live-tunable layout for the Ironhold Villager screen reskin.
 ...
 * </ul>
 */
public final class VillagerSlotDebug {
    private VillagerSlotDebug() {}

    public static final String SCREEN_KEY = "villager";
    ...
    private static int[][] makeDefaults() {
        ...
        for (int i = 0; i < CONTAINER_COUNT; i++) {
            p[i] = new int[]{ VillagerLayout.CONTAINER_SLOTS[i][0], VillagerLayout.CONTAINER_SLOTS[i][1] };
        }
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) {
                p[CONTAINER_COUNT + row * 9 + col] = new int[]{
                    VillagerLayout.INV_X + col * VillagerLayout.INV_COL_STEP,
                    VillagerLayout.INV_Y + row * VillagerLayout.INV_ROW_STEP
                };
            }
        }
        for (int col = 0; col < 9; col++) {
    ...
    private static int totalCount() {
        return SLOT_COUNT
             ...
             + ScreenWidgetTuner.labelCount(VillagerSlotDebug.SCREEN_KEY)
    ...
    private static int rangeOf(int idx) {
        ...
        if (idx < SLOT_COUNT + wc + ScreenWidgetTuner.labelCount(VillagerSlotDebug.SCREEN_KEY)) return 2;
        return 3;
    }
    ...
    private static int anchorIdx(int idx) { return idx - SLOT_COUNT - ScreenWidgetTuner.widgetCount(SCREEN_KEY) - ScreenWidgetTuner.labelCount(VillagerSlotDebug.SCREEN_KEY); }
    ...
    public static boolean tryStartDrag(int mx, int my, int leftPos, int topPos) {
        ...
        if (ai >= 0) {
            ...
            int absY = topPos + rel[1];
            int widgetCount = ScreenWidgetTuner.widgetCount(SCREEN_KEY);
            dragIndex = SLOT_COUNT + widgetCount + ScreenWidgetTuner.labelCount(VillagerSlotDebug.SCREEN_KEY) + ai;
            dragOffsetX = mx - absX;
            dragOffsetY = my - absY;
            selected = dragIndex;
            return true;
    ...
    private static String slotName(int i) {
        ...
        if (i < SLOT_COUNT + wc + ScreenWidgetTuner.labelCount(VillagerSlotDebug.SCREEN_KEY)) return ScreenWidgetTuner.labelName(labelIdx(i));
        return ScreenWidgetTuner.anchorName(SCREEN_KEY, anchorIdx(i));
    }

    public static String summary() {
        int[] p = slotPos(selected);
        return String.format("[edit] VILLAGER %d (%s) -> (%d, %d)  [total=%d]",
                selected, slotName(selected), p[0], p[1], totalCount());
    }

    public static String printJava() {
        StringBuilder sb = new StringBuilder("=== Ironhold villager layout ===\n");
        for (int i = 0; i < SLOT_COUNT; i++) {
