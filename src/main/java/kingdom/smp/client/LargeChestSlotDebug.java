...
/**
 * Live-tunable layout for the Ironhold large (6-row) chest screen. 90 slots:
 * <ul>
 *   <li>0–53  → chest grid (6×9)</li>
 *   <li>54–80 → 3×9 main inventory</li>
 *   <li>81–89 → hotbar</li>
 * </ul>
 */
public final class LargeChestSlotDebug {
    private LargeChestSlotDebug() {}
    ...
    private static int[][] makeDefaults() {
        ...
        for (int row = 0; row < 6; row++) {
            for (int col = 0; col < 9; col++) {
                p[row * 9 + col] = new int[]{LargeChestLayout.COL_X[col], LargeChestLayout.CHEST_ROW_Y[row]};
            }
        }
        // 54–80: inventory
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) {
                p[54 + row * 9 + col] = new int[]{LargeChestLayout.COL_X[col], LargeChestLayout.INV_ROW_Y[row]};
            }
        }
        // 81–89: hotbar
        for (int col = 0; col < 9; col++) {
            p[81 + col] = new int[]{LargeChestLayout.COL_X[col], LargeChestLayout.HOTBAR_Y};
        }
        return p;
    }
    ...
    private static String slotName(int i) {
        if (i <= 53) return "chest[" + i + "]";
        if (i <= 80) return "inv[" + (i - 54) + "]";
        return "hotbar[" + (i - 81) + "]";
    }

    public static String summary() {
        int[] p = positions[selected];
        return String.format("[edit] LARGE CHEST slot %d (%s) → (%d, %d)",
                selected, slotName(selected), p[0], p[1]);
    }

    public static String printJava() {
        StringBuilder sb = new StringBuilder("=== Ironhold large chest layout ===\n");
        for (int i = 0; i < SLOT_COUNT; i++) {
