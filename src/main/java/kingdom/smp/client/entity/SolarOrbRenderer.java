...
public class SolarOrbRenderer extends EntityRenderer<SolarOrbEntity, EntityRenderState> {
    ...
    public SolarOrbRenderer(EntityRendererProvider.Context ctx) {
        super(ctx);
    ...
    public void submit(EntityRenderState state, PoseStack pose, SubmitNodeCollector collector, CameraRenderState camera) {
        collector.submitModel(
            ...
            state,
            pose,
            RenderTypes.breezeWind(TEXTURE, state.ageInTicks * 0.04F % 1.0F, 0.0F),
            state.lightCoords,
