...
public class MirrorRenderer extends EntityRenderer<MirrorEntity, MirrorRenderState> {
    ...
    private static final Identifier INACTIVE_SURFACE = Identifier.fromNamespaceAndPath(kingdom.smp.Ironhold.MODID, "textures/block/mirror_surface.png");
    ...
    public boolean shouldRender(MirrorEntity entity, Frustum culler, double camX, double camY, double camZ) {
        // Don't draw mirrors into the reflection pass — the pane sits between the reflected camera
        // and the scene, so it would occlude the reflection (and recurse for other mirrors).
        if (kingdom.smp.client.MirrorReflection.isCapturing()) {
            return false;
        }
    ...
    public void extractRenderState(MirrorEntity entity, MirrorRenderState state, float partialTicks) {
        ...
        state.surfaceTexture = kingdom.smp.client.MirrorReflection.textureIdFor(entity.getId());
    }
