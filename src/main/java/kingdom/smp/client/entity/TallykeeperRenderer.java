...
import kingdom.smp.entity.TallykeeperEntity;
...
/**
 * Renderer for the Tallykeeper — vanilla villager rig with the LIBRARIAN
 * overlay (book + scroll silhouette suits a records-herald). Scale comes from
 * Attributes.SCALE on the entity.
 */
public class TallykeeperRenderer
    extends AgeableMobRenderer<TallykeeperEntity, VillagerRenderState, VillagerModel> {
    ...
    public TallykeeperRenderer(EntityRendererProvider.Context ctx) {
        super(ctx,
    ...
    @Override
    public void extractRenderState(TallykeeperEntity entity, VillagerRenderState state, float partialTick) {
        ...
        if (state.nameTagAttachment != null) {
            state.nameTagAttachment = state.nameTagAttachment.add(0.0, 0.4, 0.0);
        }
    }
}
