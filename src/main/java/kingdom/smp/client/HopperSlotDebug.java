...
/**
 * Live-tunable layout for the Ironhold Hopper screen reskin.
 ...
 * </ul>
 */
public final class HopperSlotDebug {
    private HopperSlotDebug() {}

    public static final String SCREEN_KEY = "hopper";
    ...
    private static int[][] makeDefaults() {
        ...
        for (int i = 0; i < CONTAINER_COUNT; i++) {
            p[i] = new int[]{ HopperLayout.CONTAINER_SLOTS[i][0], HopperLayout.CONTAINER_SLOTS[i][1] };
        }
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) {
                p[CONTAINER_COUNT + row * 9 + col] = new int[]{
                    HopperLayout.INV_X + col * HopperLayout.INV_COL_STEP,
                    HopperLayout.INV_Y + row * HopperLayout.INV_ROW_STEP
                };
            }
        }
        for (int col = 0; col < 9; col++) {
    ...
    private static int totalCount() {
        return SLOT_COUNT
             ...
             + ScreenWidgetTuner.labelCount(HopperSlotDebug.SCREEN_KEY)
    ...
    private static int rangeOf(int idx) {
        ...
        if (idx < SLOT_COUNT + wc + ScreenWidgetTuner.labelCount(HopperSlotDebug.SCREEN_KEY)) return 2;
        return 3;
    }
    ...
    private static int anchorIdx(int idx) { return idx - SLOT_COUNT - ScreenWidgetTuner.widgetCount(SCREEN_KEY) - ScreenWidgetTuner.labelCount(HopperSlotDebug.SCREEN_KEY); }
    ...
    public static boolean tryStartDrag(int mx, int my, int leftPos, int topPos) {
        ...
        if (ai >= 0) {
            ...
            int absY = topPos + rel[1];
            int widgetCount = ScreenWidgetTuner.widgetCount(SCREEN_KEY);
            dragIndex = SLOT_COUNT + widgetCount + ScreenWidgetTuner.labelCount(HopperSlotDebug.SCREEN_KEY) + ai;
            dragOffsetX = mx - absX;
            dragOffsetY = my - absY;
            selected = dragIndex;
            return true;
    ...
    private static String slotName(int i) {
        ...
        if (i < SLOT_COUNT + wc + ScreenWidgetTuner.labelCount(HopperSlotDebug.SCREEN_KEY)) return ScreenWidgetTuner.labelName(labelIdx(i));
        return ScreenWidgetTuner.anchorName(SCREEN_KEY, anchorIdx(i));
    }

    public static String summary() {
        int[] p = slotPos(selected);
        return String.format("[edit] HOPPER %d (%s) -> (%d, %d)  [total=%d]",
                selected, slotName(selected), p[0], p[1], totalCount());
    }

    public static String printJava() {
        StringBuilder sb = new StringBuilder("=== Ironhold hopper layout ===\n");
        for (int i = 0; i < SLOT_COUNT; i++) {
