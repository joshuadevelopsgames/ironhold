...
/**
 * Live-tunable layout for the Ironhold furnace screen. 39 slots:
 * <ul>
 *   <li>0     → input</li>
 *   <li>1     → fuel</li>
 *   <li>2     → result</li>
 ...
 * </ul>
 */
public final class FurnaceSlotDebug {
    private FurnaceSlotDebug() {}

    public static final int SLOT_COUNT = 39;
    ...
    private static int[][] makeDefaults() {
        ...
        p[0] = new int[]{FurnaceLayout.INPUT_X,  FurnaceLayout.INPUT_Y};
        p[1] = new int[]{FurnaceLayout.FUEL_X,   FurnaceLayout.FUEL_Y};
        p[2] = new int[]{FurnaceLayout.RESULT_X, FurnaceLayout.RESULT_Y};
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) {
                p[3 + row * 9 + col] = new int[]{
                        FurnaceLayout.INV_COL_X[col], FurnaceLayout.INV_ROW_Y[row]};
            }
        }
        for (int col = 0; col < 9; col++) {
            p[30 + col] = new int[]{FurnaceLayout.INV_COL_X[col], FurnaceLayout.HOTBAR_Y};
        }
        return p;
    }
    ...
    private static String slotName(int i) {
        return switch (i) {
            ...
            case 1 -> "fuel";
            case 2 -> "result";
            default -> i <= 29 ? ("inv[" + (i - 3) + "]") : ("hotbar[" + (i - 30) + "]");
        };
    }

    public static String summary() {
        int[] p = positions[selected];
        return String.format("[edit] FURNACE slot %d (%s) → (%d, %d)",
                selected, slotName(selected), p[0], p[1]);
    }

    public static String printJava() {
        StringBuilder sb = new StringBuilder("=== Ironhold furnace layout ===\n");
        for (int i = 0; i < SLOT_COUNT; i++) {
