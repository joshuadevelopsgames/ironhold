...
/**
 * Live-tunable layout for the Ironhold Grindstone screen reskin.
 ...
 * </ul>
 */
public final class GrindstoneSlotDebug {
    private GrindstoneSlotDebug() {}

    public static final String SCREEN_KEY = "grindstone";
    ...
    private static int[][] makeDefaults() {
        ...
        for (int i = 0; i < CONTAINER_COUNT; i++) {
            p[i] = new int[]{ GrindstoneLayout.CONTAINER_SLOTS[i][0], GrindstoneLayout.CONTAINER_SLOTS[i][1] };
        }
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) {
                p[CONTAINER_COUNT + row * 9 + col] = new int[]{
                    GrindstoneLayout.INV_X + col * GrindstoneLayout.INV_COL_STEP,
                    GrindstoneLayout.INV_Y + row * GrindstoneLayout.INV_ROW_STEP
                };
            }
        }
        for (int col = 0; col < 9; col++) {
    ...
    private static int totalCount() {
        return SLOT_COUNT
             ...
             + ScreenWidgetTuner.labelCount(GrindstoneSlotDebug.SCREEN_KEY)
    ...
    private static int rangeOf(int idx) {
        ...
        if (idx < SLOT_COUNT + wc + ScreenWidgetTuner.labelCount(GrindstoneSlotDebug.SCREEN_KEY)) return 2;
        return 3;
    }
    ...
    private static int anchorIdx(int idx) { return idx - SLOT_COUNT - ScreenWidgetTuner.widgetCount(SCREEN_KEY) - ScreenWidgetTuner.labelCount(GrindstoneSlotDebug.SCREEN_KEY); }
    ...
    public static boolean tryStartDrag(int mx, int my, int leftPos, int topPos) {
        ...
        if (ai >= 0) {
            ...
            int absY = topPos + rel[1];
            int widgetCount = ScreenWidgetTuner.widgetCount(SCREEN_KEY);
            dragIndex = SLOT_COUNT + widgetCount + ScreenWidgetTuner.labelCount(GrindstoneSlotDebug.SCREEN_KEY) + ai;
            dragOffsetX = mx - absX;
            dragOffsetY = my - absY;
            selected = dragIndex;
            return true;
    ...
    private static String slotName(int i) {
        ...
        if (i < SLOT_COUNT + wc + ScreenWidgetTuner.labelCount(GrindstoneSlotDebug.SCREEN_KEY)) return ScreenWidgetTuner.labelName(labelIdx(i));
        return ScreenWidgetTuner.anchorName(SCREEN_KEY, anchorIdx(i));
    }

    public static String summary() {
        int[] p = slotPos(selected);
        return String.format("[edit] GRINDSTONE %d (%s) -> (%d, %d)  [total=%d]",
                selected, slotName(selected), p[0], p[1], totalCount());
    }

    public static String printJava() {
        StringBuilder sb = new StringBuilder("=== Ironhold grindstone layout ===\n");
        for (int i = 0; i < SLOT_COUNT; i++) {
