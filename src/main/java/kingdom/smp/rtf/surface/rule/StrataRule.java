...
import kingdom.smp.rtf.tags.RTFBlockTags;
...
public record StrataRule(Identifier cacheId, int buffer, int iterations, Holder<Noise> selector, List<Layer> layers) implements SurfaceRules.RuleSource {
	public static final MapCodec<StrataRule> CODEC = RecordCodecBuilder.<StrataRule>mapCodec(instance -> instance.group(
...
//				}
	...
	public record Buffer(int size, TagKey<Block> materials) {
		public static final MapCodec<Buffer> CODEC = RecordCodecBuilder.<Buffer>mapCodec(instance -> instance.group(
